(() => {
    ...
    async function downloadArchive() {
        if (!tests.length) {
            setStatus('Добавьте хотя бы один тест перед подготовкой результата.', 'error');
            return;
        }

        const prepareArtifact = typeof window.prepareExtensionArtifact === 'function'
            ? window.prepareExtensionArtifact
            : async () => {
                throw new Error('Функция подготовки результата не реализована. Добавьте свою реализацию позже.');
            };

        const context = {
            meta: {
                requestedName: getArchiveName(),
                suggestedLabel: generateFileName(getArchiveName()),
                problemStatement: problemStatementValue,
                solutionCode: solutionCodeValue
            },
        ...
        try {
            // TODO: заменить заглушку на реальную подготовку артефакта для расширения.
            const result = await prepareArtifact(context);
            const message = result && typeof result.message === 'string'
                ? result.message
                : 'Данные переданы в заглушку подготовки результата. Реализуйте финальную логику позже.';
            setStatus(message, 'success');
        } catch (error) {
            console.error('Ошибка подготовки артефакта расширения', error);
            const errorMessage = error instanceof Error && error.message
                ? error.message
                : 'Не удалось подготовить результат. Реализуйте функцию подготовки артефакта.';
